...
class NITT(Base):
    ...
    def on_activate(self):
        ...
        self._scanner_task = self._nicard.create_ao_task(taskname = 'nitt_ao', channels = self._scanner_ao_channels, voltage_ranges = self._scanner_voltage_ranges)
    ...
    def set_up_trigger_clock(self, clock_frequency=None, clock_channel=None):
        ...
        """
        if clock_frequency is None:
            self.log.error('No clock_frequency in set_up_trigger_clock.')
            return -1
        else:
    ...
    def set_up_scanner(self):
        ...
        if self._scanner_clock_task is None:
            ...
            for i, task in enumerate(self._nicard._co_task_handles):
                if task.name == 'nitt_scanner_clock':
                    break
                elif i == len(self._nicard._co_task_handles)-1:
                    self.log.error('task nitt_scanner_clock is closed by other program')
                    return -1
        if self._scanner_ai_channels:
