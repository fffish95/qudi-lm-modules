...
class TimeTaggerLogic(GenericLogic):
    ...
    @QtCore.Slot(QCheckBox, str)
    def dumpData(self, checkBox, dumpnametext):

        if checkBox.isChecked():
            self.dumpfolder = self._save_logic.get_path_for_module(module_name='TimeTagger')
            self.dumpPath = f"{self.dumpfolder}/{dumpnametext}"
            self.dump = self._timetagger.dump(self.dumpPath,filtered_channels = 4)
            self.dump.start()
            # self.howIsTheDump()
        else:
            self.dump.stop()
            # self.howIsTheDump()
            # if f"{self.dumpPath.split('/')[-1]}_imgs" not in  os.listdir():
            #     os.makedirs(f"{self.dumpPath}_imgs")
            #     np.savetxt(f"{self.dumpPath}_imgs/hist.csv", self.hist.getData(), delimiter=",")
            #     np.savetxt(f"{self.dumpPath}_imgs/corr.csv", self.corr.getData(), delimiter=",")