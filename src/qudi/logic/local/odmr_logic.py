...
class OdmrLogic(LogicBase):
    ...
    data_scanner = Connector(interface='NITT')
    ...
    def on_activate(self):
        ...
        self._scanner = self.data_scanner()
        mw_constraints = self._microwave.constraints
        data_constraints = self._scanner._nicard._constraints
    ...
    def _initialize_odmr_data(self):
        ...
        for channel in self._scanner._channel_labelsandunits.keys():
    ...
    def data_constraints(self):
        return self._scanner._nicard._constraints
    ...
    def start_odmr_scan(self):
        ...
        with self._threadlock:
            ...
            try:
                ...
                sampler.set_up_trigger_clock(clock_frequency=sample_rate)
                sampler._set_up_trigger_line(length = samples)
    ...
    def stop_odmr_scan(self):
        ...
        with self._threadlock:
            if self.module_state() == 'locked':
                self._microwave.off()
                self._scanner.close_trigger_tasks()
                self.module_state.unlock()
            self.sigScanStateUpdated.emit(False)
    ...
    def _scan_odmr_line(self):
        ...
        with self._threadlock:
                ...
                if self._oversampling_factor > 1:
                    for ch in new_counts:
                        new_counts[ch] = np.mean(
                            list(new_counts[ch].reshape(-1, self._oversampling_factor)),
                            axis=1
                        )
            ...
            for ch, range_list in enumerate(self._raw_data.values()):
                start = 0
                for range_index, range_params in enumerate(self._scan_frequency_ranges):
                    ...
                    self.test1 = ch
                    tmp = list(new_counts[ch][start:start + range_params[-1]])
    ...
    def _get_raw_column_headers(self, data_channel):
        channel_unit = self._scanner._channel_labelsandunits[data_channel]['unit']
        return 'Frequency (Hz)', f'Scan Data ({channel_unit})'

    def _get_signal_column_headers(self):
        column_headers = ['Frequency (Hz)']
        column_headers.extend(f'{ch} ({self._scanner._channel_labelsandunits[ch]["unit"]})' for ch in self._signal_data)
    ...
    def _draw_figure(self, channel, range_index):
        ...
        unit = self._scanner._channel_labelsandunits[channel]['unit']
